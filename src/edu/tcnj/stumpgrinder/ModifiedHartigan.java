 ...
 * algorithms for species with m characters. Modified bottom-up of Hartigan's to
 * work with edge contraction algorithm.
 * 
 * @date (Spring 2016)
 * @version 1.0
 ******************************************************************************/

public class ModifiedHartigan extends Hartigan {

	/*******************************************************************************
	 * Performs a modified version of Hartigan's bottom-up algorithm. Computes
	 * VU and VL sets for all nodes except for the root node. Used with the edge
	 * contraction algorithm to update the VU and VL sets of the new tree
	 * (rooted at w) after contracting an edge.
	 * 
	 * @param tree
	 *            Tree to be computed
	 * @param worldSet
	...
	public static <T> int bottomUpRecursive(Node<List<SetList<T>>> current,
		...
		if (current.getChildren().size() >= 1) {
			...
			/*
			 * Perform regular Hartigan's on all nodes, except for the root
			 * node.
			 */
			if (current.hasParent()) {
				Pair<Integer, List<SetList<T>>> hartiganResults = hartigan(
						sets, worldSet, current);
